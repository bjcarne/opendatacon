...
{
public:
	ModbusMasterPort(const std::string& aName, Context& aParent, const std::string& aConfFilename, const Json::Value& aConfOverrides):
...
private:
	template<class T>
	ODC::CommandStatus WriteObject(const T& command, uint16_t index);
...
private:
	...
	ModbusReadGroup<ODC::Binary>* GetRange(uint16_t index);
